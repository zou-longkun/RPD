...
class TMC(nn.Module):
    ...
    def __init__(self, classes_num, lambda_epochs=1):
        """
    ...
    def DS_Combin(self, alpha):
        ...
        def DS_Combin_two(alpha1, alpha2):
            ...
            for v in range(2):
                ...
                u[v] = self.classes_num/S[v]

            # b^0 @ b^(0+1)
            bb = torch.bmm(b[0].view(-1, self.classes_num, 1), b[1].view(-1, 1, self.classes_num))
            # b^0 * u^1
    ...
    def forward(self, evidence, y, global_step=1):
        loss = 0
        alpha = dict()
        for v_num in range(len(evidence)):
            alpha[v_num] = evidence[v_num] + 1
            loss += ce_loss(y, alpha[v_num], self.classes_num, global_step, self.lambda_epochs)
        alpha_a = self.DS_Combin(alpha)
        evidence_a = alpha_a - 1
        loss += ce_loss(y, alpha_a, self.classes_num, global_step, self.lambda_epochs)
