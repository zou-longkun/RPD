    ...
    with torch.no_grad():
        for data in tqdm(trgt_train_loader):
            ...
            for i in mask[0]:
                ...
                index += 1
        io.cprint('pseudo label acc: ' + str(round(sum(np.array(pseudo_label_list) == np.array(gt_list)) / len(pc_list), 3)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='KD_new', help='Name of the experiment')
    ...
    parser.add_argument('--thd_high', type=float, default=0.7, help='thd high')
    ...
    checkpoint_io_img = checkpoints.CheckpointIO(args, model=img_model)
    checkpoint_io_pc = checkpoints.CheckpointIO(args, model=pc_model)
    try:
        load_dict_img = checkpoint_io_img.load('model_img_new.pt')
        load_dict_pc = checkpoint_io_pc.load('model_pc_new.pt')
    ...
    while True:
        ...
        for trgt_batch in tqdm(train_new_loader):
            batch_it += 1
            it += 1
            loss_ce, _, _ = trainer.model_train(trgt_batch)
            trgt_loss_ce_sum += loss_ce
        scheduler_img.step()
        scheduler_pc.step()

        io.cprint('lr_rate=%.9f' % scheduler_img.get_last_lr()[0])
        io.cprint('Train -SRC - [Epoch %02d], loss_ce=%.4f' % (epoch_it, trgt_loss_ce_sum / it))
